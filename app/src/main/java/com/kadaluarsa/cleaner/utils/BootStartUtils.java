...
public class BootStartUtils {
    ...
    public static List<AutoStartInfo> fetchAutoApps(Context mContext) {
        ...
        if (resolveInfoList.size() > 0) {
            ...
            strb = new StringBuilder(resolveInfoList.get(0).activityInfo.packageName + "/" + resolveInfoList.get(0).activityInfo.name);
            packageReceiver = strb.toString();
            ...
            for (int i = 1; i < resolveInfoList.size(); i++) {
                ...
                if (appName.equals(resolveInfoList.get(i).loadLabel(pm).toString())) {
                    strb.append(packageReceiver).append(";").append(resolveInfoList.get(i).activityInfo.packageName).append("/").append(resolveInfoList.get(i).activityInfo.name);
                    packageReceiver = strb.toString();
                } else {
                    ...
                    isenable = pm.getComponentEnabledSetting(mComponentName2) != PackageManager.COMPONENT_ENABLED_STATE_DISABLED;
                    isSystem = (resolveInfoList.get(i).activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0;
                }
